  ...
  endpoints: [
    {
      ...
      description:
        "Creates a single-use payment session for the customer. Redirect the customer to the url field in the response to open the credit card checkout. Session expires in 15 minutes.",
      paramTitle: "Parameter Reference",
      params: [
        { name: "user_id", type: "string", required: "yes", description: "Customer unique ID (callback UserCode)" },
        { name: "name", type: "string", required: "optional", description: "Customer full name (shown on payment page)" },
        { name: "amount", type: "number", required: "yes", description: "Deposit amount (TRY). Minimum is defined by site settings." },
        { name: "return_url", type: "string", required: "yes", description: "Return URL after payment" },
        { name: "transaction_id", type: "string", required: "yes", description: "External reference (callback CustomField)" },
      ],
