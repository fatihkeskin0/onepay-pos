  ...
  endpoints: [
    {
      ...
      description:
        "Müşteriye özel tek kullanımlık ödeme oturumu üretir. Yanıtta dönen url alanına müşterinizi yönlendirdiğinizde kredi kartı ödeme ekranı açılır. Oturum 45 dakika geçerlidir. amount: 0 gönderilirse müşteri tutarı ödeme sayfasında girer.",
      paramTitle: "Parametre Referansı",
      params: [
        { name: "user_id", type: "string", required: "yes", description: "Müşteri benzersiz ID (callback UserCode)" },
        { name: "name", type: "string", required: "yes", description: "Müşteri ad soyad" },
        { name: "amount", type: "number", required: "yes", description: "Yatırım tutarı (TL). 0 = serbest tutar" },
        { name: "return_url", type: "string", required: "optional", description: "Ödeme sonrası dönüş URL" },
        { name: "transaction_id", type: "string", required: "optional", description: "Harici referans (callback CustomField)" },
      ],
      ...
      notes: [
        "Serbest tutar modu için amount: 0 gönderin. Yanıtta amount_editable: true döner.",
        `Örnek serbest tutar isteği:\n${SAMPLE_CREATE_PAYMENT_LINK_FREE_REQ}`,
      ],
    },
    {
      id: "deposit-status",
