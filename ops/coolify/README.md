...
7. Set **required URL env** (do not add `SERVICE_URL_*` — not in compose):
   ...
   - Web build uses `API_PUBLIC_URL` as `NEXT_PUBLIC_API_URL` (browser + merchant API calls `https://api.onekart.info`)
...
Customer checkout (`/pay/*`) uses a **same-origin proxy** via `next.config.mjs` rewrites — no extra env, no route handler:

```
odeme.click/pay/{token}  →  browser POST /api/backend/v1/user/create_deposit
                         →  Next.js rewrite → http://api:80/v1/user/create_deposit
```
...
- **Pay page** (any web domain `/pay/*`): browser calls same-origin `/api/backend/v1/user/*`; Next.js rewrites to `http://api:80` in Docker Compose.
...
## Pre-deploy build gate (web)

`pnpm --filter @onepara/api exec tsc --noEmit` alone is **not** enough for web changes. Run the same production build as Docker before pushing:

```powershell
pnpm --filter @onepara/shared build
$env:DOCKER_BUILD="1"; pnpm --filter @onepara/web build
pnpm --filter @onepara/api exec tsc --noEmit
```

If `next build` fails, the web container will not start and all web domains return **504**.
